...
-(void)setSubViews{
    ...
    for (NSInteger i=0; i<6; i++) {//循环创建多个密码输入框
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(originX+(itemWidth+space)*i, (viewHeight-itemWidth)/2, itemWidth, itemWidth)];
    ...
    [self.textField.rac_textSignal subscribeNext:^(NSString * _Nullable x) {//监听输入内容并对应赋值
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    if (self.label1.text.length>0 && self.label2.text.length>0 && self.label3.text.length>0 && self.label4.text>0 && self.label5.text.length>0 && self.label6.text.length>0) {
        NSString *password = [NSString stringWithFormat:@"%@%@%@%@%@%@",self.label1.text,self.label2.text,self.label3.text,self.label4.text,self.label5.text,self.label6.text];
